<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PHP7</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Zenh87qX5JnK2Jl0vWa8Ck2rdkQ2Bzep5IDxbcnCeuOxjzrPF/et3URy9Bv1WTRi" crossorigin="anonymous">

    <style>
        dt { margin: 10px 0; }
        dd { text-indent: 20px; }
    </style>
</head>
<body>

<div class="container py-3">

    <?php

    // Função do tipo void: função sem retorno;
    function exibirBoasVindas() {

        echo 'Bem-vindo ao curso de PHP!';
    }

    exibirBoasVindas();

    // Função com retorno
    function calcularAreaTerreno($largura, $comprimento) {

        $area = $largura * $comprimento;
        return $area;
    }

    $AreaTerreno = calcularAreaTerreno(10, 20);

    ?>

    <header class="mt-4">
        <h1>Funções</h1>
        <p>Encapsular um bloco de códigos com um objetivo definido</p>
    </header>
    <hr>

    <section>
        <div class="card w-25 shadow-sm">
            <div class="card-header text-center">
                <div class="py-2 fw-bold fs-5">Calcular Área do Terreno</div>
            </div>
            
            <div class="card-text px-3 text-center">
                <div class="py-2"><?= $AreaTerreno ?>m<sup>2</sup></div>
            </div>
        </div>
    </section>

</div>


<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
    
</body>
</html>